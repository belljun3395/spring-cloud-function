...
az group create --name my-eventgrid-rg --location eastus

# Create storage account (required for Azure Functions)
az storage account create \
  --name mystorageaccount123 \
  --resource-group my-eventgrid-rg \
  --location eastus \
  --sku Standard_LRS

# Create Azure Function App
az functionapp create \
  --name my-eventgrid-function \
...
----
{
  "IsEncrypted": false,
  "Values": {
    "AzureWebJobsStorage": "DefaultEndpointsProtocol=https;AccountName=mystorageaccount123;AccountKey=<YOUR_KEY>;EndpointSuffix=core.windows.net",
...
az eventgrid event-subscription create \
  ...
  --source-resource-id /subscriptions/{subscription-id}/resourceGroups/my-eventgrid-rg/providers/Microsoft.EventGrid/topics/my-eventgrid-topic \
...
az eventgrid event-subscription create \
  ...
  --endpoint "${FUNCTION_URL}" \
  --event-delivery-schema cloudeventschemav1_0 \
  --deadletter-endpoint /subscriptions/{subscription-id}/resourceGroups/my-eventgrid-rg/providers/Microsoft.Storage/storageAccounts/mystorageaccount123/blobServices/default/containers/deadletter
----
